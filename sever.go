...
func main() {
	...
	flag.StringVar(&data, "data", "", "Data to POST. If present HTb will use POST requests instead of GET")
	flag.Var(&headers, "header", "Add headers to the request. Could be used multiple times")
	flag.Parse()
	t := os.Getenv("HoTieuBao Attacking")
	...
	go func() {
		fmt.Println("-- HTB Attack Started --\n           Go!\n\n")
